...
var Form = _ui.default.inherit({
  ...
  _createHiddenElement: function _createHiddenElement(rootLayoutManager) {
    this._$hiddenElement = (0, _renderer.default)('<div>').addClass(WIDGET_CLASS).addClass(HIDDEN_LABEL_CLASS).appendTo('body');
  ...
  _renderValidationSummary: function _renderValidationSummary() {
    ...
    if (this.option('showValidationSummary')) {
      var _$validationSummary = (0, _renderer.default)('<div>').addClass(FORM_VALIDATION_SUMMARY).appendTo(this._getContent());
  ...
  _itemTabbedTemplate: function _itemTabbedTemplate(item, e, $container) {
    ...
    (0, _renderer.default)($container).addClass(FIELD_ITEM_CONTENT_HAS_TABS_CLASS);
  ...
  _itemGroupTemplate: function _itemGroupTemplate(item, e, $container) {
    var $group = (0, _renderer.default)('<div>').toggleClass(FORM_GROUP_WITH_CAPTION_CLASS, (0, _type.isDefined)(item.caption) && item.caption.length).addClass(FORM_GROUP_CLASS).appendTo($container);
    (0, _renderer.default)($container).addClass(FIELD_ITEM_CONTENT_HAS_GROUP_CLASS);
    var colCount;
    var layoutManager;

    if (item.caption) {
      (0, _renderer.default)('<span>').addClass(FORM_GROUP_CAPTION_CLASS).text(item.caption).appendTo($group);
    }

    var $groupContent = (0, _renderer.default)('<div>').addClass(FORM_GROUP_CONTENT_CLASS).appendTo($group);
