...
var Calendar = _editor.default.inherit({
  ...
  _supportedKeys: function _supportedKeys() {
    return (0, _extend.extend)(this.callBase(), {
      ...
      home: function home(e) {
        ...
        var date = _date.default.sameView(zoomLevel, currentDate, min) ? min : _date.default.getViewFirstCellDate(zoomLevel, currentDate);
        ...
        var date = _date.default.sameView(zoomLevel, currentDate, max) ? max : _date.default.getViewLastCellDate(zoomLevel, currentDate);
  ...
  _moveToClosestAvailableDate: function _moveToClosestAvailableDate() {
    ...
    var currentDate = _date.default.createDate(baseDate);
    ...
    var dateForward = _date.default.createDate(currentDate);

    var dateBackward = _date.default.createDate(currentDate);
  ...
  _updateCurrentDate: function _updateCurrentDate(date) {
    ...
    if (date.getTime() !== normalizedDate.getTime()) {
      this.option('currentDate', _date.default.createDate(normalizedDate));
      return;
    }
  ...
  _getDateByOffset: function _getDateByOffset(offset, date) {
    ...
    var difference = _date.default.getDifferenceInMonth(this.option('zoomLevel')) * offset;
    date.setDate(1);
    date.setMonth(date.getMonth() + difference);

    var lastDay = _date.default.getLastMonthDate(date).getDate();
  ...
  _isViewAvailable: function _isViewAvailable(date) {
    ...
    var min = _date.default.getViewMinBoundaryDate(zoomLevel, this._getMinDate());

    var max = _date.default.getViewMaxBoundaryDate(zoomLevel, this._getMaxDate());
  ...
  _cellClickHandler: function _cellClickHandler(e) {
    ...
    var nextView = _date.default.getViewDown(zoomLevel);
  ...
  _updateTimeComponent: function _updateTimeComponent(date) {
    var result = _date.default.createDate(date);
  ...
  _navigateUp: function _navigateUp() {
    ...
    var nextView = _date.default.getViewUp(zoomLevel);
  ...
  _isDateInInvalidRange: function _isDateInInvalidRange(date) {
    ...
    var normalizedDate = _date.default.normalizeDate(date, min, max);
  ...
  _getDate: function _getDate(value) {
    var result = _date.default.createDate(value);
  ...
  _optionChanged: function _optionChanged(args) {
    ...
    switch (args.name) {
      ...
      case 'value':
        ...
        this.option('currentDate', (0, _type.isDefined)(value) ? _date.default.createDate(value) : new Date());
