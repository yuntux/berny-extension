...
function populateAxesInfo(axes) {
  return axes.reduce(function (result, axis) {
    ...
    if (majorTicks && majorTicks.length > 0 && (0, _type.isNumeric)(majorTicks[0]) && options.type !== 'discrete' && !businessRange.isEmpty() && !(businessRange.breaks && businessRange.breaks.length) && axis.getViewport().action !== 'zoom') {
      axis.applyMargins();
